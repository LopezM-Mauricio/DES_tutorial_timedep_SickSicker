...
trace_plot <- ggplot(dt_CT_trace_long, aes(x = T_n, y = Proportion, color = Health_State)) +
  geom_line(linewidth = 1.3) +
  
  # Titles and Labels
  xlab("Time (years)") +
  ylab("State Occupancy (Proportion)") +
  labs(title = "Mean State Occupancy over Time", color = "Health States") +
...
trace_plot <- ggplot(dt_DT_trace_long, aes(x = T_n, y = Proportion, color = Health_State)) +
  geom_line(linewidth = 1.3) +
